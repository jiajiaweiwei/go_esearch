...
import (
	"context"
	"errors"
	"framework/internal/server/proxy/load_balancer"
...
type EtcdServiceHub struct {
	...
	loadBalancer       load_balancer.LoadBalancer // 策略模式 负载均衡策略的接口，支持多种负载均衡实现 这里实现了轮询和随机
}

const (
	ServiceRootPath = "/go-search" // etcd key的前缀
)
...
func GetServiceHub(etcdServers []string, heartbeatFrequency int64) *EtcdServiceHub {
	...
	if etcdServiceHub == nil {
		...
		hubOnce.Do(func() {
			...
			// 这里有bug,如果没安装etcd,这里居然不报错，err是nil的，导致实际使用etcd时会panic，注意：开启webserver服务前需要手动开启etcd服务，否则会异常，etcd没有设置自启动
			client, err := etcdv3.New(etcdv3.Config{
				Endpoints:   etcdServers,                         // etcd 服务器的地址列表
				DialTimeout: 3 * time.Second,                     // 连接超时时间
				DialOptions: []grpc.DialOption{grpc.WithBlock()}, // todo 因为ETCD为搜索框架的关键服务，所以这里配置为第一次链接失败就退出程序
			})
			if err != nil {
			...
			etcdServiceHub = &EtcdServiceHub{
				...
				loadBalancer:       &load_balancer.RoundRobin{}, // 这里选择使用Round-Robin负载均衡策略
			}
		})
	}
