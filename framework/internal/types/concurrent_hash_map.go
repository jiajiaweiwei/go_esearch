...
// PartitionedRWMutexMap 自行实现并发读写的map。key是string，value是any。
type PartitionedRWMutexMap struct {
...
// NewPartitionedRWMutexMap 创建并返回一个新的 PartitionedRWMutexMap 实例。
...
//
// 返回值:
//   - *PartitionedRWMutexMap: 一个新的 PartitionedRWMutexMap 实例。
func NewPartitionedRWMutexMap(seg, cap int) *PartitionedRWMutexMap {
	...
	// 返回创建的 PartitionedRWMutexMap 实例。
	return &PartitionedRWMutexMap{
		mps:   mps,
		seg:   seg,
		locks: locks,
		seed:  0,
...
//   - key: 键，表示要存储的key。
//   - value: 值，表示要存储的value。
func (m *PartitionedRWMutexMap) Set(key string, value any) {
	// 根据key计算出应该存储的分片索引
...
//   - bool: 表示key是否存在。
func (m *PartitionedRWMutexMap) Get(key string) (any, bool) {
	// 根据key计算出存储的分片索引
	index := m.getSegIndex(key)
...
//
// 返回值:
//   - *PartitionedRWMutexMapIterator: 迭代器对象，用于遍历PartitionedRWMutexMap中的键值对。
func (m *PartitionedRWMutexMap) CreateIterator() *PartitionedRWMutexMapIterator {
	...
	return &PartitionedRWMutexMapIterator{
		cm:       m,
		keys:     keys,
		rowIndex: 0,
		colIndex: 0,
...
// 返回值:
//   - int: 计算出的分片索引。
func (m *PartitionedRWMutexMap) getSegIndex(key string) int {
