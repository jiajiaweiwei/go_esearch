...
type SkipListInvertedIndexer struct {
	table *PartitionedRWMutexMap // 使用分段锁保护的并发安全 map，用于存储倒排索引的数据
...
func NewSkipListInvertedIndexer(docNumEstimate int) *SkipListInvertedIndexer {
	indexer := &SkipListInvertedIndexer{
		...
		table: NewPartitionedRWMutexMap(runtime.NumCPU(), docNumEstimate),
...
//
// 参数:
//   - doc: 需要添加的文档，类型为 .Document。
func (indexer *SkipListInvertedIndexer) Add(doc Document) {
...
//
// 参数:
//   - keyword: 要删除的文档的关键词，类型为 *.Keyword。
//   - IntId: 要删除的文档的唯一标识符，类型为 uint64。
func (indexer *SkipListInvertedIndexer) Delete(keyword *Keyword, IntId uint64) {
...
func (indexer *SkipListInvertedIndexer) Search(query *TermQuery, onFlag uint64, offFlag uint64, orFlags []uint64) []string {
	// 执行搜索并获取匹配的 SkipList
...
func (indexer *SkipListInvertedIndexer) search(q *TermQuery, onFlag uint64, offFlag uint64, orFlags []uint64) *skiplist.SkipList {
	// 处理叶子节点情况，即直接根据关键词查找。
	if q.Keyword != nil {
		// 获取关键词对应的跳表。
