...
import (
	"context"
	"fmt"
	"framework/frame/server/proxy/service_hub"
...
//
// 参数:
//   - doc: 要添加的文档，类型为 util_types.Document。
//
// 返回值:
//   - int: 成功添加的文档数量。
//   - error: 如果在添加文档时出现错误，返回相应的错误信息。
func (sentinel *Sentinel) AddDoc(doc util_types.Document) (int, error) {
...
//  6. 返回存储的文档列表。
func (sentinel *Sentinel) Search(query *util_types.TermQuery, onFlag, offFlag uint64, orFlags []uint64) []*util_types.Document {
	// 获取该服务所有的 endpoints
	endpoints := sentinel.hub.GetServiceEndpoints(IndexService)
	if len(endpoints) == 0 {
		return nil
	...
	// 用于存储检索结果的切片和通道
	docs := make([]*util_types.Document, 0, 1000)
	resultChan := make(chan *util_types.Document, 1000)
